  ...
  const filterAvailableTables = React.useCallback(async () => {
    ...
    try {
      ...
      const reservedTableIds = (reservations || [])
        .filter(r => r.table_id !== reservation.table_id)
        .map(r => r.table_id)
      setAvailableTables(
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  <span className="text-sm font-semibold">{formData.date}</span>
                </button>
                ...
                >
                  ...
                  <span className="text-sm font-semibold">
                    {formData.party_size}{' '}
                    {parseInt(formData.party_size) === 1 ? t.person : t.people}
                  </span>
                </button>

                {/* Table chip */}
                {(() => {
                  const selected = availableTables.find(
                    tb => tb.id === formData.table_id
                  )
                  return (
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span className="text-sm font-semibold">
                        {selected ? selected.table_identifier : t.selectTable}
                      </span>
                    </button>
                  )
                })()}
              ...
              {activeStep === 'table' && (
                <div className="rounded-xl !bg-emerald-50 p-4">
                  <StepTable
                    ...
                    onChange={tableId => {
                      setFormData(fd => ({ ...fd, table_id: tableId }))
                      setActiveStep(null)
                    }}
                    loading={loadingTables}
                    noTablesMessage={t.noTablesConfigured}
                    seatsLabel={t.seats}
                  />
                </div>
              )}
